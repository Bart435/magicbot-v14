...
module.exports = {
	...
	execute(client) {
		let timer = setInterval(function () {
			connection.query(
				`SELECT * FROM discman.posts WHERE slug = 'wipe10x'`,
				function (err, results) {
					...
					for (let i = 0; i < results.length; i++) {
						...
						connection.query(
							`SELECT * FROM discman.wipes WHERE slug = '10x'`,
							function (err, results) {
								if (err) throw err;
								setTimer(
									messageID,
		...
		function setTimer(
			...
			const embed = new EmbedBuilder()
				.setTitle("Magic Ark Basic (10X servers)")
				.setDescription(
					`**Previous wipe date** : ${
						last_timestamp == 0
							? "Not available"
					...
					}\n\n** Beginners server wipe**: ${dateHelpers.toDiscordTimestampDateMonth(
						nextWipeDate
					)}`
				)
				.addFields(
					{
						name: "Settings :",
						value: "<#1116743660764139642>",
						inline: true,
					},
					{
						name: "Join links :",
						value: "<#1131983508043214858>",
						inline: true,
					},
					{
						name: "Server rules :",
						value: "<#1116745977269850223>",
						inline: true,
					}
				)
			...
			if (wipe_timestamp != 0) {
				if (wipe_timestamp <= now) {
					connection.query(
						`UPDATE discman.wipes SET wipe_timestamp = '0', last_timestamp = '${wipe_timestamp}' WHERE slug = '10x'`,
						function (err) {
							if (err) throw err;
						}
					);
